...
void Model::Draw(Shader shader,GLuint instancedVBO)
{
	for (GLuint i = 0; i < this->meshes.size(); i++)
	{
		//this->meshes[i].Draw(shader,instancedVBO);
	}
...
void Model::loadModel(string path)
{
	...
	{
		Error::Message(0,importer.GetErrorString());
		return;
	}
...
GLuint Model::TextureFromFile(const char* path, string directory)
{
	...
	unsigned char * image = SOIL_load_image(filename.c_str(),&width,&height,&channels,SOIL_LOAD_RGBA); //Load texture
